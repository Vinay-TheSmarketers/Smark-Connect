...
body.report-diagnostic .analysis-section{border-top-color:var(--purple)}body.report-atlas .module-opener{background:linear-gradient(155deg,var(--soft) 0 42%,#fff 42%)}body.report-field-guide .executive-item{border-radius:24px}body.report-studio .analysis-section.prominent{background:linear-gradient(135deg,#fff,var(--soft))}body.report-playbook .roadmap-item{border-left:5px solid var(--purple)}body.report-editorial .section-kicker,body.report-product-book .section-kicker{text-transform:uppercase}.qa-compact .analysis-section{padding:13px}.qa-compact html{font-size:10px}
...
<body class="report-{{ profile.layout }} {{ body_class }}" style="--purple:{{ profile.accent }};--deep:{{ profile.deep }};--soft:{{ profile.soft }};--blush:{{ profile.soft }}">
