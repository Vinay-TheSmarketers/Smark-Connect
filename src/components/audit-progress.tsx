...
export function AuditProgress({ jobId, initial }: { jobId: string; initial: JobState }) {
  ...
  return (
    <div className="form-card audit-progress-card">
      ...
      <div className="audit-checks"><div className={job.progress >= 8 ? "done" : ""}><span>01</span>Website safety check</div><div className={job.progress >= 28 ? "done" : ""}><span>02</span>Research crawl</div><div className={job.documents.length >= 6 ? "done" : ""}><span>03</span>Six core documents</div><div className={job.progress >= 91 ? "done" : ""}><span>04</span>Agents + CMO synthesis</div></div>
      <div className="document-pipeline"><div className="pipeline-heading"><strong>Concurrent document generation</strong><span>{job.documents.length}/6 ready</span></div>{documentPipeline.map(([type, label]) => { const ready = job.documents.some((document) => document.type === type); const running = !ready && job.progress >= 42 && job.status === "RUNNING"; return <div className={ready ? "ready" : running ? "running" : "queued"} key={type}><ModuleIcon type={type} size={15} /><strong>{label}</strong><small>{ready ? "Ready" : running ? "Analyzing" : "Queued"}</small><span>{ready ? "✓" : running ? "●" : "○"}</span></div>; })}</div>
      ...
      {!finished && job.status !== "ERROR" && <p className="submit-note">This work continues on the server if you refresh or leave the page.</p>}
    </div>
  );
}
