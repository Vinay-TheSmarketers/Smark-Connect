...
type AuditItem = { strategy: string; performance: number | null; accessibility: number | null; bestPractices: number | null; seo: number | null; lcp: number | null; fcp: number | null; tbt: number | null; cls: number | null; statusCode: number | null; responseTime: number | null; ttfb: number | null; transferSize: number | null; source: string; error: string | null; createdAt: string };
type DashboardData = {
  company: { id: string; name: string; websiteUrl: string; logoUrl: string | null; category: string | null; description: string | null; companyBrief: string; lastAuditedAt: string | null };
...
function transferSize(value: number | null) { return value === null ? "—" : value >= 1024 * 1024 ? `${(value / 1024 / 1024).toFixed(1)}MB` : `${Math.round(value / 1024)}KB`; }
...
export function DashboardClient({ data }: { data: DashboardData }) {
  ...
  const simpleTiming = audit?.source === "Python URL timing test";
  ...
  return <main className="dashboard-shell">
    ...
    <section className="dashboard-grid" ref={workspaceRef} style={gridStyle}>
      <aside className="company-pane pane" {...paneProps("context")}>
        ...
        <section className="pane-section context-competitors"><div className="section-label-row"><p className="section-label">COMPETITORS</p><span>{competitorItems.length ? `${competitorItems.length} verified` : "Discovery pending"}</span></div>{competitorItems.length ? <div className="context-competitor-grid">{competitorItems.map((item, index) => <ContextCompetitor item={item} key={`${item.companyName || item.title}-${index}`} />)}</div> : <div className="context-competitor-empty"><span className="extended-document-icon"><Sparkles size={13} /></span><div><strong>Build the competitor set</strong><small>Run the competitor agent to discover six real companies and their official logos.</small></div><button type="button" disabled={Boolean(runningAgent)} onClick={() => runAgent("COMPETITOR")}>{runningAgent === "COMPETITOR" ? "Finding…" : "Find competitors"}</button></div>}</section>
        {paneResizer("context")}
      </aside>
      ...
      <section className="analytics-pane pane" {...paneProps("analytics")}>
        ...
        <div className="analytics-content">
          ...
          {(analysisTab === "seo" || analysisTab === "technical") && <>
            ...
            <div className="source-banner"><div><Gauge size={18} /><span><strong>{audit?.source ?? "Python URL timing test"}</strong><small>{simpleTiming ? "Lightweight server-response check" : "Saved browser-lab snapshot"}</small></span></div><span className={audit?.error ? "source-status error" : "source-status"}>{audit?.error ? "Unavailable" : simpleTiming ? "Local test" : "Saved"}</span></div>
            <p className="audited-line">Last audited: {data.company.lastAuditedAt ? formatTimestamp(data.company.lastAuditedAt) : "Not yet"}</p>
            <div className="section-heading"><div><strong>{simpleTiming ? "URL Response Timing" : analysisTab === "technical" ? "Technical Page Experience" : "Saved PageSpeed Scores"}</strong><small>{simpleTiming ? "Mobile and desktop user-agent requests are timed separately" : "Historical browser-lab evidence"}</small></div><div className="strategy-toggle"><button className={strategy === "desktop" ? "active" : ""} onClick={() => setStrategy("desktop")}>Desktop</button><button className={strategy === "mobile" ? "active" : ""} onClick={() => setStrategy("mobile")}>Mobile</button></div></div>
            {audit?.error ? <div className="analytics-error"><strong>The URL timing test is unavailable.</strong><span>{audit.error}</span></div> : simpleTiming ? <><div className="score-card"><ScoreGauge label="Response score" score={audit?.performance ?? null} /></div><div className="vitals-grid"><div><span className="metric-dot" />HTTP<strong>{audit?.statusCode ?? "—"}</strong><small>Final response</small></div><div><span className="metric-dot" />TTFB<strong>{metric(audit?.ttfb ?? null, "ms")}</strong><small>Headers received</small></div><div><span className="metric-dot" />TOTAL<strong>{metric(audit?.responseTime ?? null, "ms")}</strong><small>HTML transfer</small></div><div><span className="metric-dot" />SIZE<strong>{transferSize(audit?.transferSize ?? null)}</strong><small>Captured HTML</small></div></div><div className="field-data-note"><strong>What this test does—and does not measure</strong><p>The Python check measures HTTP status, time to response headers, total HTML transfer time, and transfer size. The response score is a transparent heuristic, not Google PageSpeed or Lighthouse. It does not measure rendering, JavaScript, accessibility, SEO, or Core Web Vitals; Google PageSpeed can be connected later.</p></div></> : <><div className="score-card"><ScoreGauge label="Performance" score={audit?.performance ?? null} /><ScoreGauge label="Accessibility" score={audit?.accessibility ?? null} /><ScoreGauge label="Best Practices" score={audit?.bestPractices ?? null} /><ScoreGauge label="SEO" score={audit?.seo ?? null} /></div><div className="section-heading"><div><strong>Core Web Vitals</strong><small>Saved Lighthouse lab metrics</small></div></div><div className="vitals-grid"><div><span className="metric-dot" />LCP<strong>{metric(audit?.lcp ?? null, "s", 1000)}</strong><small>{audit?.lcp && audit.lcp <= 2500 ? "Pass" : "Review"}</small></div><div><span className="metric-dot" />FCP<strong>{metric(audit?.fcp ?? null, "s", 1000)}</strong><small>{audit?.fcp && audit.fcp <= 1800 ? "Pass" : "Review"}</small></div><div><span className="metric-dot" />TBT<strong>{metric(audit?.tbt ?? null, "ms")}</strong><small>{audit?.tbt !== null && audit?.tbt !== undefined && audit.tbt <= 200 ? "Pass" : "Review"}</small></div><div><span className="metric-dot" />CLS<strong>{metric(audit?.cls ?? null, "")}</strong><small>{audit?.cls !== null && audit?.cls !== undefined && audit.cls <= .1 ? "Pass" : "Review"}</small></div></div><div className="vitals-diagnosis">{(["lcp", "fcp", "tbt", "cls"] as const).map((kind) => { const detail = vitalDetail(kind, audit?.[kind] ?? null); return <article className={detail.state === "Good" ? "good" : detail.state === "Unavailable" ? "muted" : "review"} key={kind}><span>{kind.toUpperCase()}</span><strong>{detail.state}</strong><p>{detail.detail}</p></article>; })}</div></>}
            ...
            <div className="audit-footnote">Timing-test observations are kept separate from browser lab data, field Core Web Vitals, and connected first-party analytics.</div>
          </>}
